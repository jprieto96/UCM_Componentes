package integracion.lineaFactura.modificarLineaFactura;
...
public class TestIntegracionModificarLineaFactura {
	...
	public static void before() { //Antes de cada Test
		...
		producto1 = new TProducto(1, "Producto1", 99, 20, 1, true);
	...
	@Test
	public void modificarLineaDeFactura(){
		...
		t.commit();
		//Modifico la linea de factura
		lineaFactura.setCantidad(6);
		lineaFactura.setPrecio(producto1.getPrecio() * lineaFactura.getCantidad());
		t = dameTransaccion();
		t.start();
		daoLineaFactura.update(lineaFactura); 
		t.commit();
		//La leo
		t = dameTransaccion();
		t.start();
		...
		//Ahora comprobamos todos sus campos se han modificado correctamente, de la unica linea de venta que hemos creado y posteriormente modificado
		while(it.hasNext()){
			TLineaFactura aux;
			aux = it.next();
			assertEquals(lineaFactura.getCantidad(), aux.getCantidad());
