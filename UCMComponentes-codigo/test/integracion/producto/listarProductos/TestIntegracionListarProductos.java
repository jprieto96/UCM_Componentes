package integracion.producto.listarProductos;
...
public class TestIntegracionListarProductos {
	...
	public void listarProductos(){
		...
		t = dameTransaccion();
		t.start();
		Collection<TProducto> collect = daoProducto.readAll();//Lo leo
		t.commit();
		Iterator<TProducto> it = collect.iterator();
		
		while(it.hasNext()){
			TProducto aux;
			aux = (TProducto) it.next();
			...
			aux = (TProducto) it.next();
			//Ahora comprobamos que todos los campos se han introducido correctamente
			assertEquals(producto2.getNombre(), aux.getNombre());
			assertEquals(producto2.getPrecio(), aux.getPrecio(), 0.0);
			assertEquals(producto2.getExistencias(), aux.getExistencias());
			assertEquals(producto2.getIdProveedor(), aux.getIdProveedor());
			assertEquals(true, aux.isActivo());
